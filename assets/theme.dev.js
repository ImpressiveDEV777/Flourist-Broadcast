          ...
          if (this.cartDrawerEnabled) {
            ...
            setTimeout(() => {
              if (button !== null) {
                ...
                if (button && button.closest(this.settings.elements.pairProductsHolder)) {
                  setTimeout(() => {
...
//               }
            ...
            this.html.dispatchEvent(
              new CustomEvent('cart:add-item', {
                bubbles: true,
                detail: {
                  selector: quickAddHolder,
                },
              })
            );
      
            if (response.handle == "tip") {
              location.reload();
            }
    ...
    updateCart(updateData = {}, holder = null, valueIsEmpty = false) {
      ...
      fetch(theme.routes.root + 'cart.js')
        ...
        .then((response) => {
          ...
          if(response.items[index] && response.items[index].title) {
    ...
    openCartDropdown() {
      document.querySelector('[data-site-container]').classList.add('active');
      document.getElementById('shopify-section-footer').classList.add('active');
    ...
    closeCartDropdown() {
      document.querySelector('[data-site-container]').classList.remove('active');
      document.getElementById('shopify-section-footer').classList.remove('active');
    ...
    build(data) {
      ...
      if (localStorage.getItem('method') == '3') {
        var vcartItems = document.querySelectorAll('.cart__items .cart-item'), flag = true;
    ...
    freeShippingMessageHandle(total) {
      if (this.cartMessage.length > 0) {
        this.document.querySelectorAll(this.settings.elements.cartMessage).forEach((message) => {
          ...
          const cartMessageClass = hasFreeShipping ? this.settings.classes.success : 'is-visible';
    ...
    show() {
      ...
      if (this.buttonQuickAddMobile) {
        this.buttonQuickAddMobile.addEventListener('click', () => {
          ...
          var postalCodeM = localStorage.getItem("postalcode");
                                                   	
		  if (postalCodeM) {
            if (postalCodeM != "Shipping" || this.quickAddLabel.dataset.tag == "Shipping") {                                                   
            ...
            setTimeout(function() {
              document.getElementById('methodDropdown').classList.add('active');
            }, 300);
            return;
          }
        });
      ...
      if (this.quickAddLabel) {
        ...
        this.quickAddLabel.addEventListener('click', () => {
          ...
          if (postalCode) {
            if (postalCode != "Shipping" || this.quickAddLabel.dataset.tag == "Shipping") {
  ...
  const selectors$q = {
    ...
    articleTemplate: '[article-grid-item-template]',
  ...
  class SearchPredictive {
    constructor(input) {
      ...
      this.articleTemplate = document.querySelector(selectors$q.articleTemplate).innerHTML;
      ...
      this.articleWrapper = this.append.querySelector(selectors$q.articleWrapper);
    ...
    render(terms) {
      fetch(`/search/suggest.json?q=${encodeURIComponent(terms)}&resources[type]=product&resources[limit]=6&resources[options][unavailable_products]=last`)
        .then(this.handleErrors)
        .then((response) => response.json())
        .then((response) => {
          this.productResult = response.resources.results;
        ...
        .then((response) => {
          ...
          fetch(`/search/suggest.json?q=${encodeURIComponent(terms)}&resources[type]=article&resources[limit]=6&resources[options][unavailable_articles]=last`)
            .then(this.handleErrors)
            .then((response) => response.json())
            .then((response) => {
              this.articleResult = response.resources.results;
              return this.fetchArticles(response.resources.results.articles);
            })
            .then((response) => {
    //           this.injectTitle(terms);
...
//                 this.reset(false);
    ...
    injectTitle(terms) {
      ...
      let count = '';
      if (this.productResult && this.productResult.products.length > 0) {
        count = this.productResult.products.length;
    ...
    injectProduct(productHTML) {
      if (productHTML) {
      	this.productWrapper.innerHTML += "<h4>Products</h4>";  
      }
      this.productWrapper.innerHTML += productHTML;
    }
	
	injectArticle(articleHTML) {
      this.articleWrapper.innerHTML = '';
      if (articleHTML) {
      	this.articleWrapper.innerHTML += "<h4>Recipes &amp; Articles</h4>";  
      }
      this.articleWrapper.innerHTML += articleHTML;
    }

	fetchArticles(articles) {
      const promises = [];
      articles.forEach((article) => {
        promises.push(this.renderArticle(article));
      });
    ...
    renderArticle(article) {
      return Sqrl.render(this.articleTemplate, {article: article});
    }
